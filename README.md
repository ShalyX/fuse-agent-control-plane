...
- Policy-bound workload classes with per-call, invocation, aggregate branch-budget, and minimum-input constraints. A caller-selected class cannot expand the immutable branch allowlist.
- Immutable mandate branches with parent identity, agent binding, policy-version binding, lifetime budget, expiry, centralized-authority disclosure, and a canonical delegation hash that is recomputed on authorization reads.
- Explicit decision evidence for branch limit, committed spend, request reservation, maximum exposure, and remaining authority. Bounded exposure is not labeled acceptable exposure.
- Reconciled-cost sibling-divergence evidence in shadow mode. Authoritative completion commits first; a durable best-effort queue isolates shadow failures from responses and mandate state. It cannot deny, trip, or reduce authority; see [`docs/sibling-divergence-shadow-v1.md`](docs/sibling-divergence-shadow-v1.md).
...
- `POST /api/v1/admin/mandates/:mandateId/branches` with `mandates:admin`; branches can be created only while the mandate is `draft` or `paused`.
...
- `GET /api/v1/admin/mandates/:mandateId/shadow-evaluations` with `policies:read`.
...
With `DATABASE_URL` configured, authenticated `/v1/chat/completions` requests use policy admission, transactional reservation, provider invocation, and usage reconciliation. Workload-scoped policies require both `X-Fuse-Branch` and `workload_class`; the branch and class are included in the canonical idempotency fingerprint. OpenRouter cannot start without that controlled path. The no-database direct-Anthropic route remains a legacy compatibility mode and is not part of the policy-control evidence.

`FUSE_WORKLOAD_SHADOW_ENABLED` defaults to `false`. When false, the supported HTTP surface rejects workload-policy publication, branch creation, and workload-scoped inference while preserving legacy unscoped inference and ceiling-only Console policy publication. Production activation uses two deployments: first deploy this branch-aware binary with the flag off and `DATABASE_URL_UNPOOLED`, confirm `/ready` reports `workloadShadowSchema: true`, and make it the rollback target; only then enable the flag. The pre-branch binary is not a safe rollback target for active workload-bound mandates.

The branch `delegationHash` is generated by Fuse over a centralized control-plane record. It is not a customer signature and does not prove external authorization. Delegation signing and payment signing are both part of the unresolved custody/authority design; settlement and payment orchestration remain closed.

The control-plane operating contract is:

- `/health` is process liveness. `/ready` fails closed when its checker is absent or empty and verifies PostgreSQL, provider configuration, policy schema v4/v5 (`workloadShadowSchema`), and the durable PostgreSQL sandbox store. It returns `503` without database error details when any dependency is unavailable.
- Schema bootstrap requires `DATABASE_URL_UNPOOLED` (or an already-unpooled `DATABASE_URL`) because its session advisory lock is incompatible with Neon transaction pooling.
- Schedule `npm run shadow:retry` against the unpooled URL to process durable shadow failures. Each item is attempted at most three times; the command prints pending, retryable, exhausted, and completed counts without credentials.
- `FUSE_ADMIN_RATE_LIMIT_PER_MINUTE` defaults to 120 administrative requests per authenticated organization/principal. Production runtime enforcement is PostgreSQL-backed, so limits survive restarts and coordinate across serverless instances. Authentication runs before capacity is consumed; forwarding headers are not trusted. Policy-controlled inference has a separate durable per-mandate request limit.
- Run `npm run migrate:provider-config` with production `DATABASE_URL_UNPOOLED` and the provider credential key ring before activating tenant-provider mode. Runtime bootstrap remains an idempotent fallback, not the preferred deployment migration path.
...
- Application rollback uses Vercel promotion of the last known-good **branch-aware** deployment or a Git revert. Never roll active workload-bound mandates back to the pre-branch binary; pause them first if an emergency requires that downgrade. Provider schema v1 and policy schemas v4/v5 are additive; do not delete their tables or retire old KEKs during application rollback. After rollback, smoke `/health`, `/ready`, authenticated identity, public persisted state, workload rollout gating, and reconciliation visibility.
- `npm run test:credential-recovery-postgres` runs the recovery rollback and two-connection race against an explicitly configured unpooled URL, or creates and removes a disposable local PostgreSQL database. The test uses an isolated schema and verifies cleanup. It never prints the connection URL.
- `npm run test:workspace-onboarding-postgres` proves stale-operation leasing, heartbeat renewal, atomic tenant-scoped cleanup, and retry authority against an explicitly configured unpooled URL or disposable local PostgreSQL database.
- `npm run smoke:production` performs the non-destructive public and auth-boundary smoke suite. CI runs tests, TypeScript build, real PostgreSQL credential and workspace-onboarding recovery gates, production dependency audit, and a high-confidence credential-material scan. The separate Neon persistence pull-request gate verifies provider races, workload-shadow persistence, both recovery paths, and held-out reliability persistence against the hosted unpooled database.

The repository also contains an independently deployable signer boundary and operator tools:

- `npm run ops:check` calls authenticated `/api/v1/admin/readiness`, reports control-mode, settlement-disablement, durable invite/limiter/session-revocation, stale-onboarding, health, and reconciliation status without printing credentials, and exits nonzero on any unsafe invariant. `npm run ops:beta-alerts` emits the same invite-only control-plane failures as alert names; it has no signer or payment-capacity dependency.
